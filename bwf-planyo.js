...
(function () {
  ...
  var FUNCTION_URL = "https://" + PROJECT + ".supabase.co/functions/v1/planyo-bridge";
  var accessToken = "";
  ...
  function tokenUitOpslag() {
    ...
    try {
      for (var i = 0; i < localStorage.length; i++) {
        ...
        if (sleutel.indexOf("sb-") !== 0 || sleutel.indexOf("-auth-token") < 0) continue;
        var waarde = JSON.parse(localStorage.getItem(sleutel) || "null");
        var sessie = Array.isArray(waarde) ? waarde[0] : waarde;
        if (sessie && sessie.access_token) return sessie.access_token;
      }
    } catch (e) {}
    return "";
  }
  ...
  async function verzoek(actie, parameters) {
    ...
    if (!token) throw new Error("Log eerst in om de Planyo-agenda te laden.");

    var url = new URL(FUNCTION_URL);
    url.searchParams.set("action", actie);
    Object.keys(parameters || {}).forEach(function (naam) {
      if (parameters[naam] != null && parameters[naam] !== "") {
    ...
    if (!antwoord.ok) throw new Error(data.error || "Planyo kon niet worden geladen (" + antwoord.status + ").");
    return data;
  }

  async function verstuur(actie, gegevens) {
    var token = tokenUitOpslag();
    if (!token) throw new Error("Log eerst in om Planyo te gebruiken.");
    var antwoord = await fetch(FUNCTION_URL, {
      ...
      body: JSON.stringify(Object.assign({ action: actie }, gegevens || {}))
    });
    var data = await antwoord.json().catch(function () { return {}; });
    if (!antwoord.ok) throw new Error(data.error || "Planyo-opdracht mislukte (" + antwoord.status + ").");
    return data;
  }

  window.BWFPlanyo = {
    setAccessToken: function (token) { accessToken = token || ""; },
    reservations: function (van, tot) { return verzoek("reservations", { from: van, to: tot }); },
    customers: function (pagina) { return verzoek("customers", { page: pagina || 0 }); },
