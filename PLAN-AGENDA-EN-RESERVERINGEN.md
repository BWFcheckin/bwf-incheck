...
- [x] Reserveringsdetail: gast, verblijf, geld, bronmateriaal, koppelingen, wijzigingslog (12-09-2026). Besluit Angela: **geen aparte `reservering.html`**, maar uitbreiden van `reservering-beheer.html` — daar werkt Kelly al, en anders zouden dezelfde velden op twee plekken onderhouden moeten worden. Het venster heeft nu ook: **bijlagen** (pdf, foto, e-mail of Excel; upload naar de niet-openbare bucket `reservering-bijlagen`, vastgelegd in `reserveringen.bron_bestanden`, openen via een tijdelijke link — toevoegen mag eigenaar en VR, bekijken iedereen met toegang), **koppelingen** (klant, incheckformulier, welkomstcall, nachtregister, schade, met een knop naar de juiste pagina) en de **wijzigingslog** uit `reservering_log`.
- [x] Plak-/uploadveld in het reserveringsdetail (12-09-2026, `b771e07`): pdf via pdf.js, txt/csv/eml rechtstreeks, Excel via SheetJS — alle drie pas geladen zodra zo'n bestand langskomt. De tekst gaat in het plakveld (alleen als dat leeg is) en daarna werkt "Gegevens herkennen" erop; het bestand zelf komt in de bucket `reservering-bijlagen` en in `reserveringen.bron_bestanden`. Geplakte tekst gaat bij opslaan naar `brongegevens`.
- [ ] Restant plak-/uploadveld: hetzelfde veld ook in de welkomstcall (`vr2.html` heeft nu een eigen bijlage-upload naar de bucket `documenten`, zonder tekstherkenning), en tekst uit een foto (OCR, Tesseract) — nu wordt een foto alleen bewaard.
- [x] Incheckformulieren met drie staten: **klaargezet** (concept), **nieuw/leeg**, **definitief** (12-09-2026). Staat op de koppelingsregel "Incheckformulier" in het reserveringsdetail; het formulier wordt gevonden via `reserveringen.checkin_id` of via `checkins.gekoppelde_reservering = reservering:<uuid>`, dat het incheckformulier sinds deze fase wegschrijft.
- [x] `incheckformulier.html`: openen, bewerken, opslaan, koppelen aan reservering; concept toewijzen aan locatiemanager; gastlink genereren (12-09-2026). Openen, bewerken, opslaan, toewijzen (`c_manager`) en de gastenlink bestonden al; nieuw is het koppelen aan de reservering — bij opslaan wordt `reserveringen.checkin_id` gezet en `checkins.gekoppelde_reservering` op `reservering:<uuid>`, waardoor het detail de drie staten kan tonen.
- [x] Welkomstcall: reservering kiezen → extra's bijboeken → WhatsApp/e-mail → Mollie-link voor restant/extra's (12-09-2026, `27673a7`). De betaallink rekende alleen de bijgeboekte extra's; nu restant (uit `reserveringen`) + extra's + tijdstoeslag, met de opbouw in beeld. De betaling gaat met `reservering_id`, `welkomstcall_id` en `soort` mee naar Mollie, zodat de webhook hem aan de boeking hangt. Bericht via WhatsApp of Gmail bestond al. **Bevinding:** `mollie-webhook` stond op Verify JWT en gaf elke melding van Mollie een 401 — daarom was `mollie_betalingen` leeg en kwam er nooit een betaalstatus terug. Na akkoord van Angela staat de inlogcontrole alleen op die functie uit (versie 11); de functie vertrouwt de melding niet en haalt de betaling zelf op bij Mollie.
- [x] Klant aanmaken/samenvoegen in `wz_klantbeheer`; overnachting doorzetten naar nachtregister (12-09-2026). De knop in het incheckformulier maakte bij elk bezoek een nieuwe regel `ci-<id>`; nu wordt eerst gezocht op e-mailadres en anders op de laatste negen cijfers van het telefoonnummer, en een gevonden klant bijgewerkt (lege velden overschrijven nooit iets dat al gevuld is). **Bevinding:** het "nachtregister voor de toeristenbelasting" is in de praktijk `wz_klantbeheer` met `type = 'overnachting'` (7 van de 21 klanten). De aparte tabel `wz_gastenregister` is **leeg** en wordt alleen in `vr2.html` genoemd — beslissen of die weg kan of juist de echte registratie moet worden (§5 opruimlijst).
- [x] Bekende bugs meenemen (12-09-2026): **arrangementen/betaling niet opgeslagen** — bij heropenen zette `laadInConcept` het opgeslagen totaal in het handmatige totaalveld, en dat veld wint altijd van de berekening; alles wat je daarna toevoegde telde niet meer mee. Veld wordt niet meer voorgevuld, een verschil wordt gemeld. Ook: de keuzelijsten voor betaalwijze staan verborgen, dus stond er altijd "Contant"; de wijze komt nu uit de ingevoerde betalingen (`ed33322`). **Gastformulier leegt velden en handtekening** — twee oorzaken: "Formulier leegmaken" annuleren liet het id van de vorige gast staan waardoor het eerstvolgende opslaan diens reservering overschreef (`8304a9e`), en het tekenvel toonde de handtekening van de gast niet, waardoor er een nieuwe overheen werd gezet (`ce78d77`). `gast.html` overschrijft niet meer wat de gast al typte. **Geen bedrag bij aanmaken** — opslaan met € 0,00 wordt geblokkeerd in beide paden; zonder bedrag stuurde de Planyo-brug `custom_price` niet mee en rekende Planyo zijn eigen tarief (`ce78d77`).
- [x] Gewenste functies uit het verwijderde `archief/dashboard-test.html` terugbrengen in `dashboard.html` (12-09-2026, `5cedbc9`): (1) handmatige in-/uitchecktijd bleek er al te zijn en schrijft nu naar `reserveringen.incheck_tijd`/`uitcheck_tijd`; (2) de kaart toonde alleen "Gedaan"/"Nog te doen" en toont nu de echte stand (nog te bellen, ingepland, niet bereikbaar, afgerond) met datum en wie belde; (3) "Besproken" stond alleen in het belvenster en staat nu ook op de kaart zelf. Oude code: `git show 5d26838:archief/dashboard-test.html`
...
- [x] Taak openen in `vr2.html` (14-09-2026) — venster `dlgTaak` met titel, toelichting, deadline, prioriteit en toegewezen persoon, te openen met de potloodknop per taak. Hiervoor gold: taak openen bestond nog niet: een taak heeft alleen de knoppen overdragen, bezig, afvinken, heropenen en verwijderen. Venster bouwen om een taak te bekijken en te wijzigen (`wz_taken`: titel, toelichting, deadline, prioriteit, toegewezen persoon, link).
...
| Reservering bewerken | gastgegevens, personen en in-/uitchecktijden (eigen suites) | ✔ | ✔ |
...
- 12-09-2026 — rest van fase 4 gestart op verzoek van Angela (detailpagina, plak-/uploadveld, Mollie-flow, bekende bugs). Besluiten: het reserveringsdetail komt in `reservering-beheer.html` (waar Kelly al werkt) en niet in een aparte `reservering.html`; Mollie gaat via één spoor (`mollie-create-payment` + `mollie-webhook`) met de reservering erbij; de wijzigingslog legt ook vast wat de import verandert. Bekende bugs opgelost — de ernstigste: wie bij "Formulier leegmaken" op Annuleren klikte, schreef de gegevens en handtekening van de volgende gast over de reservering van de vorige. Bij de verkenning bleek verder dat er twee losse Mollie-sporen bestonden die niets van elkaar wisten (`planyo-bridge` + `bwf_betalingen` naast `mollie-create-payment` + `mollie_betalingen`), dat de betaallink in de welkomstcall alleen de bijgeboekte extra's rekende (niet het restant en niet de tijdstoeslag), en dat `incheckformulier.html` meerdere dode kopieën van functies bevat waarvan alleen de laatste telt — daar is bij elke fix op gelet. De broncode van de gedeployde Mollie-functies stond alleen in `exports/` en staat nu in `supabase/functions/`.
- 12-09-2026 — migratie 9 (`20260912100000_fase4_log_en_mollie.sql`, met terugdraaiscript) na proefdraai met rollback uitgevoerd: tabel `reservering_log` (per veld wie wat wanneer veranderde, gevuld door een trigger, ook bij de import met bron `import`; lezen mag wie de reservering mag zien, niemand schrijft er rechtstreeks in) en `reservering_id`, `welkomstcall_id` en `soort` op `mollie_betalingen`. Proef: één wijziging gaf twee logregels (`personen: 2 → 3`, `notitie: leeg → …`). `mollie-create-payment` en `mollie-webhook` opnieuw gedeployed (versie 10 resp. 11). Controle achteraf: de webhook antwoordt nu met "Ongeldig Mollie-betalingsnummer" in plaats van een 401, dus Mollie kan hem bereiken; `kanalen-sync` en `mollie-webhook` zijn de enige functies zonder inlogcontrole, de rest staat aan.
- 12-09-2026 — rest van fase 4 gebouwd: reserveringsdetail in `reservering-beheer.html` met bijlagen, koppelingen en wijzigingslog (`f5b10b8`), tekst uit een bijlage in het plakveld via pdf.js (`b771e07`), klant samenvoegen en de drie staten van het incheckformulier (`4b32089`). De wijzigingslog werkt in de praktijk: 13 regels op de eerste dag, allemaal van Angela (bedrag, betaalstatus, e-mailadres, "Laat inchecken 22:00", één annulering) en géén van de import — dus hij loopt niet vol. Aandachtspunt bij het lezen van deze code: `incheckformulier.html` bevat meerdere dode kopieën van functies (`clearCheckinForm`, `openGuestModal`, `editCheckinFromDetail`, `renderGuestStatus`) waarvan alleen de láátste telt; elke fix is daarop gecontroleerd.
- 13-09-2026 — **oude gegevens overgezet** (`supabase/migrations/20260913090000_oude_gegevens_overzetten.sql`, `5488153`), na proefdraai met rollback en een export vooraf in `exports/2026-09-13-voor-migratie/`. 29 nieuwe reserveringen uit `reservations` (23 kanaal `planyo` met het Planyo-nummer als `kanaal_ref`, 6 kanaal `eigen`); 8 rijen overgeslagen omdat dezelfde boeking al bestond (zelfde suite, dag en gastnaam). 14 bestaande reserveringen aangevuld uit `res_koppeling` (alleen lege velden), 3 welkomstcalls extra gekoppeld. Totaal 135 → 164. `reservations` (37) en `res_koppeling` (78) staan onaangeroerd. Let op: de tabel eist `vertrek > aankomst`; bij dagverblijven waar de uitchecktijd vóór de inchecktijd lag is drie uur aangehouden.
- 13-09-2026 — **één pagina `reserveringen.html`** (`b7e48cf`): `reserveringen.html`, `reservering-aanmaken.html`, `reservering-beheer.html` en het vr2-tabblad samengevoegd. De schone `reservering-beheer` is de basis geworden; tabs Lijst (nu ook een periodefilter), Reservering (het bestaande venster), Nieuwe reservering (kanaal handmatig/eigen, tijdsblok kiezen) en Incheck (formulier klaargezet/definitief/geen, gastlink). De oude pagina's verwijzen door met hun parameters; `?blokkade=1` gaat naar de agenda. Oude versies bewaard in `exports/2026-09-13-oude-paginas/`. **Niet meegenomen uit de oude pagina:** klantkaart (staat in Klantbeheer), zwarte lijst, reviewberichten, wijzigings- en annuleringsberichten en de maandkalender — zeggen als die terug moeten.
- 13-09-2026 — **locatiemanager mag beperkt bewerken** (besluit Angela; migratie `20260913120000_locatiemanager_beperkt_bewerken.sql`, met terugdraaiscript). Aanleiding: een proef als Ruth liet zien dat zij 107 reserveringen ziet (jacuzzi + deluxe, 0 van angie) maar niets kon wijzigen — dat klopte met §5, maar Angela wil dat ze gastgegevens en tijden kan aanvullen. RLS kan niet per kolom, dus: de bewerkregel laat een locatiemanager toe voor de eigen suites, en de trigger `bwf_reserveringen_beperkt` zet bij die rol alle andere velden terug op hun oude waarde. Het scherm mag dus gewoon het hele formulier opsturen. Proef bevestigd: naam en inchecktijd gewijzigd, bedrag (€ 2594), status en notitie ongewijzigd, en 0 rijen bij Suite Angie. `reserveringen.html` schakelt dezelfde velden vrij en verbergt Bevestigd. Het incheckformulier meldt nu wanneer koppelen aan de reservering niet mocht (dat mislukte stil).
- 13-09-2026 — meldingen Angela opgelost: (1) **dubbele menuknop** in `vr2.html` — naast het tabblad "Reserveringen & incheck" stond een losse link met dezelfde naam en zónder `data-groep`; die gaf de tabwissel-logica een lege groep, waardoor welkomstcalls en taken niet meer te openen waren. Link weg, en de drie menu-ingangen (Reserveringen, Reservering aanmaken, Incheckformulier) zijn er één geworden. Gecontroleerd dat het níét aan de rechten lag: Kelly mag calls bijwerken en taken aanmaken, afvinken én verwijderen, en alle zestien lijsten van vr2 laden goed. (2) **Inchecktijden** stonden leeg omdat het veld alleen een *afwijking* van het tijdsblok toonde; ze worden nu voorgevuld met de tijd van de boeking. (3) **Dubbele reserveringen verwijderen** kan nu (migratie `20260913140000`): eigenaar en VR mogen verwijderen, en een trigger legt vóór het verwijderen kanaal, nummer, suite, datum en gastnaam vast in `reservering_log`. Let op: staat de boeking nog in de feed van een kanaal, dan zet de import hem terug — dan is annuleren de juiste weg; verwijderen is voor de Planyo-dubbelen uit de migratie. (4) **Menubalk**: op schermen vanaf 1200px staat het menu vast aan de zijkant naast de bovenbalk; smaller blijft het uitklapmenu.
- 14-09-2026 — **taken en welkomstcalls bewerken/verwijderen** (vraag Angela). Bevonden: een taak had alleen overdragen, bezig, afvinken en heropenen — titel, toelichting, deadline, prioriteit en toegewezen persoon waren nergens te wijzigen; en de verwijderknoppen bij taken én welkomstcalls stonden achter `isEigenaar()`, terwijl §5 zegt dat VR taken mag verwijderen en de database dat ook toestaat (eigenaar, vr, of de eigenaar van de taak). Opgelost: venster `dlgTaak` via een potloodknop per taak, en de verwijderknoppen staan er voor iedereen — wie het niet mag wordt door de database geweigerd. Welkomstcalls waren al te openen, te wijzigen en op te slaan. Vooraf gecontroleerd met een teruggedraaide proef als Kelly: call bijwerken, taak aanmaken, afvinken en verwijderen lukken alle vier, en alle zestien lijsten van vr2 laden goed — het lag dus niet aan rechten of data.
- 13-09-2026 — **één bron voor alle pagina's** (`punt 4`): geen enkele pagina leest of schrijft nog in `res_koppeling` of `reservations`; de Google Agenda en Planyo zijn nergens meer een bron. Eén reserveringsnummer overal: `kanaal_ref`, anders de eerste acht tekens van het id (`bwf-kalender.js` v=5, `reserveringen.html`, incheck-link). De daglijst toonde het nummer alleen bij SMG en toont het nu bij elk kanaal.
