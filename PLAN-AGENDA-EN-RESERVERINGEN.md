...
2. **Handmatige SMG-regel** → reservering met het kanaal uit de titel ("Booking" → `booking`, "Origineel" → `oo`, "Planyo" → `planyo`, anders `handmatig`), `kanaal_ref = smg-regel:<UID>`. Titel met "niet beschikbaar" of "Vol" → blokkade.
3. **Booking.com** → reservering (`overnachting`) tot en met 14 nachten, `kanaal_ref` = UID; langer → blokkade.
...
7. **Bijwerken:** tijden, type en tijdsblok komen steeds uit de feed; gastgegevens en `brongegevens` worden alleen ingevuld als ze nog leeg zijn (handmatige aanvullingen blijven staan). Arrangementen, extra's en bedragen raakt de import niet aan.
...
- [x] Migratie 6 `20260911130100_fase2_cron.sql` (uitgevoerd 11-09-2026; hetzelfde token staat als secret `KANALEN_SYNC_TOKEN` bij de Edge Function): token in Vault + `pg_cron`-job elke 15 minuten via `pg_net`
...
10. ~~Booking.com-feed als reservering of blokkade?~~ **Besloten 11-09-2026:** reservering tot en met 14 nachten, langer als blokkade (fase 2, regel 3).
